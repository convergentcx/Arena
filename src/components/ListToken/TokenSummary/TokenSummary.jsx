...
class TokenSummary extends Component {
  ...
  getFixedTokenSummaryData = async () => {
    ...
    (await contract) &&
      contract.methods.name().call({ from: this.props.account }, (error, result) => {
        name = result;
        this.setState({ name: name });
      });
    (await contract) &&
      contract.methods.exponent().call({ from: this.props.account }, (error, result) => {
        exponent = result;
        this.setState({ exponent: exponent });
      });
    (await contract) &&
      contract.methods.inverseSlope().call({ from: this.props.account }, (error, result) => {
        inverseSlope = result;
        this.setState({ inverseSlope: inverseSlope });
      });

    (await contract) &&
      contract.methods.totalSupply().call({ from: this.props.account }, (error, result) => {
    ...
    (await contract) &&
      contract.methods
        ...
        .call({ from: this.props.account }, (error, result) => {
  ...
  render() {
    ...
    return (
      <tr onClick={this.showDetails} className={classes.th}>
        ...
        <td>
          <Row>{this.state.action1}</Row>
          <Row>{this.state.action2}</Row>
          <Row>{this.state.action3}</Row>
        </td>
        <td>
          <Button color="success" onClick={this.showDetails}>
            Invest
          </Button>
        </td>
        <td>
          <Button color="info" onClick={this.showDetails}>
            Chat
          </Button>
        </td>
      </tr>
