...
class BuySell extends Component {
  ...
  buyAmountChangeHandler = event => {
    ...
    this.props.contract &&
      this.props.contract.methods
        .priceToMint(`${String(event.target.value * multiplier)}`)
  ...
  sellAmountChangeHandler = event => {
    ...
    this.props.contract &&
      this.props.contract.methods
        .rewardForBurn(`${event.target.value * multiplier}`)
        .call({ from: this.props.account }, (error, result) => {
    ...
    return (
      <Row className={classes.Row}>
        <Col md={6}>
          <InputGroup>
            <InputGroupAddon addonType="prepend">
              <Button color="success" onClick={this.onBuyHandler}>
                Buy
              </Button>
            </InputGroupAddon>
            <Input
            ...
            />
            <InputGroupAddon addonType="append">{`With ${(
              this.state.priceInEther / multiplier
            ).toFixed(3)} ETH`}</InputGroupAddon>
          </InputGroup>
        ...
        <Col md={6}>
          <InputGroup>
            <InputGroupAddon addonType="prepend">
              <Button color="danger" onClick={this.onSellHandler}>
                Sell
              </Button>
            </InputGroupAddon>
            <Input
            ...
            />
            <InputGroupAddon addonType="prepend">{`For ${this.state.rewardInEther.toFixed(
              3
            )} ETH`}</InputGroupAddon>
          </InputGroup>
