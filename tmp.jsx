...
class RequestService extends React.Component {
  ...
  onRequestWithEthHandler = serviceIndex => {
    ...
    const stackId =
      contract &&
      contract.methods['requestWithEth'].cacheSend(
        `${this.state.message}`,
  ...
  onRequestWithTokenHandler = () => {
    ...
    const stackId =
      contract &&
      contract.methods['requestWithToken'].cacheSend(`${this.state.message}`, {
        from: account
      });
      ...
      if (action[1] === '') {
        ...
        return (
          <tr key={i}>
            ...
            <td>
              {' '}
              <Input value={this.state.message} onChange={this.inputChangedHandler} />
            </td>
            <td>
              <Button color="danger" onClick={() => this.onRequestWithEthHandler(i + 1)}>
                Request with {this.props.requestPrices[requestPriceAttribute]} ETH
              </Button>
            </td>
            <td>
              <Button color="success" onClick={this.onRequestWithTokenHandler}>
                Request with {this.props.prices[priceAttribute]} {this.props.symbol}
              </Button>
            </td>
          </tr>
        );
